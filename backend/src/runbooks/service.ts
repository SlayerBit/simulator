...
export async function captureRunbookForSimulation(simulationId: string): Promise<void> {
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        action: 'event.runbook_generated',
        simulationId,
        metadata: {
          type: 'runbook_generated',
          runbook_id: (payload as any)?.runbook_id ?? null,
          message: `Runbook generated by Agent 1 (${incidentType})`,
          severity: 'success',
        } as any,
      },
    });
    const redisStatus = (payload as any)?.handoff?.redis;
    if (redisStatus?.published) {
      await prisma.auditLog.create({
        data: {
          action: 'event.runbook_sent_redis',
          simulationId,
          metadata: {
            type: 'runbook_sent_redis',
            runbook_id: (payload as any)?.runbook_id ?? null,
            queue: redisStatus.queue ?? 'runbooks',
            message: `Runbook sent to Redis queue ${redisStatus.queue ?? 'runbooks'}`,
            severity: 'info',
          } as any,
        },
      });
