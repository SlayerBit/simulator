...
export const api = {
  ...
  me: (token: string) =>
    request<{ authenticated: boolean; user?: any }>('/api/auth/me', { method: 'GET', token }),

  // Simulations
  listSimulations: (token: string) =>
    request<{ simulations: any[] }>('/api/simulations', { method: 'GET', token }),
  createSimulation: (token: string, body: any) =>
    request<{ simulation: any }>('/api/simulations', {
      method: 'POST',
      token,
      body: JSON.stringify(body),
    }),
  stopSimulation: (token: string, id: string) =>
    request<{ id: string; status: string }>(`/api/simulations/${id}/stop`, {
      method: 'POST',
      token,
    }),
  ...
  rollbackSimulation: (token: string, id: string) =>
    request<{ id: string; status: string }>(`/api/simulations/${id}/rollback`, {
      method: 'POST',
      token,
    }),
  getMethodsMeta: (token: string) =>
    request<{ methods: any[] }>('/api/simulations/meta', { method: 'GET', token }),

  // Templates
  listTemplates: (token: string) =>
    request<{ templates: any[] }>('/api/templates', { method: 'GET', token }),
  getTemplate: (token: string, id: string) =>
    request<{ template: any }>(`/api/templates/${id}`, { method: 'GET', token }),
  ...
  listSchedules: (token: string) =>
    request<{ schedules: any[] }>('/api/schedules', { method: 'GET', token }),
  ...
  // Dependencies
  listDependencies: (token: string) =>
    request<{ services: string[]; edges: any[] }>('/api/dependencies', { method: 'GET', token }),
  createDependency: (
    token: string,
    body: { fromService: string; toService: string; description?: string }
  ) =>
    request<{ edge: any }>('/api/dependencies', {
      method: 'POST',
      token,
      body: JSON.stringify(body),
    }),
